...
class TheLead(unittest.TestCase):
    ...
    def test_a_long_lead_with_a_short_first_sentence_passes(self):
        """Conta la prima frase, non il lead: è quella che finisce in SERP."""
...
class Figures(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # l'anno scritto accanto è giudicata contro **quell'anno**.
    ...
    def test_an_explicit_other_year_is_judged_against_that_year(self):
        """Non più saltata: controllata contro l'anno che nomina.
        ...
        nuova produce in quantità: gli angoli del pacchetto sono quasi tutti
        nel tempo.
        """
...
class HistoricalFiguresAreCheckedNotSkipped(unittest.TestCase):
    ...
    molte più cifre di anni passati di quante ne citasse quella vecchia, e
    ...
    def setUp(self):
        ...
        # `territories` e non `observations`: la mappa dei nomi si legge da lì,
        # perché `observations` copre solo l'ultimo anno e un territorio con
        ...
        # versione del codice che non esiste più.
    ...
    def test_the_value_first_syntax_honours_the_year_too(self):
        ...
        accanto però lo leggeva **solo** il secondo: la stessa affermazione
        vera passava girata in un modo e prendeva un `cifra-falsa` girata
        nell'altro, cioè un blocco che rimanda chi scrive a correggere un
        numero corretto. È il verso che l'officina usa più spesso, perché un
    ...
    def test_a_threshold_honours_the_year_as_well(self):
        ...
        dell'articolo, cioè `soglia-falsa` su una frase corretta.
        """
    ...
    def test_a_threshold_is_not_also_read_as_a_value(self):
        ...
        e il ramo giusto taceva. È la stessa soppressione che
        `states_a_value` fa già per "circa" e "quasi", dall'altro lato.
        """
    ...
    def test_the_two_syntaxes_agree_on_every_case(self):
        """La proprietà, non i due esempi: girare la frase non cambia il verdetto."""
...
class DynamicsMustCiteASource(unittest.TestCase):
    ...
    def _with_corpus(self, ids, esistenti=None):
        ...
        quando un'affermazione esiste ma non riguarda questa serie, che è il
    ...
    def _dinamica_che_cita(self, *ids):
        """Un'entry con l'identificatore attaccato **alla dinamica**, che è la
    ...
    def test_a_corpus_field_alone_does_not_satisfy_a_positional_check(self):
        ...
        Il campo `corpus` a livello di entry è **obbligatorio** nello schema
        ...
        il contrario, cioè era il difetto scritto come aspettativa."""
        self._with_corpus(["a", "b"])
    ...
    def test_a_claim_on_another_section_does_not_cover_the_dynamics(self):
        """Stessa regola, l'altra metà: l'attribuzione ha un posto, e un
    ...
    def test_a_wrong_citation_is_caught_wherever_it_hangs(self):
        ...
        fonte inventata o fuori tema è un difetto della pagina pubblica
    ...
    def test_citing_an_id_that_does_not_exist_blocks(self):
        """Il modo più facile di zittire un lint è inventare la fonte."""
        self._with_corpus(["a"])
    ...
    def test_a_real_quote_about_another_indicator_blocks(self):
        ...
        Nella prima run la citazione Eurostat sulla sensibilità ciclica della
        disoccupazione di lunga durata è finita a spiegare il **tasso di
        attività**, perché condividono il tema. La frase esiste, l'URL c'è,
        la verifica come stringa passa. E l'attribuzione è falsa lo stesso.
        """
        ...
        self.assertEqual(found[0]["severity"], lint.BLOCKS,
                         "un'attribuzione falsa su una pagina pubblica non è un avviso")
...
class Severities(unittest.TestCase):
    ...
    def test_a_rule_that_blocks_is_about_truth_not_taste(self):
        """Il gusto non blocca. È ciò che la rubrica a venti punti sbagliava."""
...
class TheDefinitionIsCheckedAgainstTheSource(unittest.TestCase):
    ...
    Questa confronta ciò che l'articolo dice di misurare con le parole della
    fonte, ed è la classe di errore che sopravvive a ogni lettura perché
    l'aritmetica intorno è giusta: `ter-30` spiegava una domanda per circuito
    ...
    def test_an_article_that_paraphrases_the_source_passes(self):
        buona = ("Istat divide il numero di visitatori dei circuiti per il totale "
                 ...
                 "che esce è un carico medio per istituto.")
        self.assertEqual(self._rilievi(buona), [])

    def test_a_missing_denominator_is_flagged_not_blocked(self):
        """Trentacinque articoli del catalogo stanno qui, ed è l'arretrato
    ...
    def test_an_indicator_without_official_metadata_is_silent(self):
        """`scoperto` descrive la nostra copertura, non l'onestà dell'articolo:
        ...
        cadeva non perché la regola fosse rotta, ma perché la copertura era
    ...
    def test_an_omitted_definition_section_does_not_fire(self):
        """La macchina nuova omette la definizione quando l'angolo più forte non
        è definitorio, ed è il comportamento voluto: `assente` scatta su 149
...
class TheAngleHasToBeOneThatWasDetected(unittest.TestCase):
    ...
    resterebbe un'istruzione che nessuno controlla, cioè una speranza.

    Il pacchetto è finto apposta. Il vero costa una vista completa
    ...
    def test_an_article_that_declares_nothing_is_left_alone(self):
        """`angolo` è facoltativo: 2 articoli su 377 lo scrivono. La regola
        tace sugli altri 375 senza costruire un solo pacchetto, che è anche
        perché `lint_all` resta a undici secondi."""
    ...
    def test_an_indicator_with_no_pack_is_silent_not_red(self):
        """Un pacchetto che non si costruisce è un guasto di dati, non un
