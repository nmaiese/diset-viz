...
class EveryIndicatorPageRenders(unittest.TestCase):
    ...
    That is the whole point of the refactor and also its main risk: a field that
    is None for one family only (a province-only BES series has no catalog
    metadata, so no source label) breaks a page no targeted test happens to open.
    Exactly that shipped a 500 during this rebuild, caught only because an
    unrelated assertion loaded the page.

    Rendering all of them costs about five seconds, which is worth paying.
    """
    ...
    def _get(self, indicator_id, query=""):
        family, raw_id = UnifiedViewReproducesEveryFigure._family_and_raw(indicator_id)
