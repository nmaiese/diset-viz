"""Lo store degli articoli letto **committato**, cioè tutto intero.
...
di un giro reale, e la suite veloce deve restare la suite della logica. Finché
...
catalogo: un test che non fallisce mai per la ragione che dichiara è comunque
...
class TheCommittedStore(unittest.TestCase):
    def test_no_key_in_the_catalogue_breaks_the_assumption(self):
        """La codifica delle chiavi regge perché nessuna chiave vera contiene
        già il separatore. La garanzia sta nel catalogo, non nella funzione:
        è l'unico dei due test che può scoprire una chiave nuova che arriva."""
