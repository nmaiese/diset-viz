...
## L'ipotesi di partenza, e perche' non ha retto

Thäsler-Kordonouri, Thurman, Schwertberger e Stalph, *Journalism* 26(9) 2025,
esperimento su **3.135 lettori** e 24 articoli data-driven, meta' scritti a mano
e meta' automatizzati: comprensibilita' 76,79 contro 73,10, sulla parte numerica
74,07 contro 69,38, e **nessuna differenza significativa su stile e struttura
delle frasi**. I due mediatori del divario sono la **densita' numerica** e la
**scelta delle parole**.

Sembrava la spiegazione del freddo, e su questo corpus **non e' cosi'**.
Misurata la densita' numerica, numeri ogni cento parole:

    per articolo      q10    q25    q50    q75    q90
    esempi (10)      1.63   2.00   3.54   6.80  11.54
    pubblicati (376) 2.03   2.44   2.91   3.62   4.78

I nostri articoli **non sono piu' densi**: la mediana e' piu' bassa. Lo studio
misurava NLG post-editato in inglese, noi abbiamo prosa di modello in italiano,
e la soglia non si trasferisce. Sta scritto qui perche' un'ipotesi provata e
caduta vale quanto una confermata, e perche' senza questo paragrafo qualcuno
la riproverebbe.
...
qualcosa senza portare niente. Non e' densita' sbagliata, e' **separazione**:
...
Ed e' lo stesso difetto che il lint aveva gia' trovato da un'altra porta, con
`dinamica-senza-fonte` su 52 articoli su 52: un paragrafo senza una cifra e
senza un identificatore di corpus non e' sobrio, e' **vuoto**. E' il freddo,
contato.

## Perche' sugli esempi e non sui 52 riscritti

I 52 sono i freddi. Calibrarci sopra consacrerebbe il difetto invece di
misurarlo. `content/esempi/` e' un campione piccolo ma della popolazione
giusta: testi che un italiano ha scritto e che qualcuno ha letto volentieri.
Stesso principio di `packs/calibration.py`: e' una misura, e ritoccarla a mano
...
# Un numero e' una cifra con la sua eventuale parte decimale, virgola o punto.
...
# Sotto i tre paragrafi la quota e' 0, 1/2 o 1, e non misura niente.
MIN_PARAGRAPHS = 3
...
def read_exemplar(path: str) -> str:
    ...
    date e URL, cioe' numeri che non sono prosa e falserebbero la misura verso
...
def build() -> dict:
    ...
    return {
        ...
        "perche": (
            "Ipotesi provata e caduta: la densita' numerica di "
            "Thäsler-Kordonouri et al. (Journalism 26(9) 2025, n=3135) non "
            "separa i due corpora, i nostri articoli non sono piu' densi. "
        ...
        # La soglia e' il q90 del campione buono: sopra, il paragrafo che
        # afferma senza portare niente e' la regola invece dell'eccezione.
