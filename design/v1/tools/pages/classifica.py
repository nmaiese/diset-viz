...
Il contesto della rotta porta la classifica gia' calcolata dall'app (punteggio,
dimensioni piu' forti e piu' deboli, classifiche per dimensione, conteggi delle
fonti). Qui si compongono le frasi e le righe che la pagina 1.0 chiede in piu':
la risposta con prima, ultima e distanza, il titolo-affermazione sul
Mezzogiorno, le tessere, le righe della tabella con la barra 0-100, la mappa.

Due cose il contesto non le porta e si chiedono all'app:
- gli anni dei dati, che `build_bes_ranking` non espone ancora. Si leggono
  dagli stessi indicatori che il punteggio usa e si accettano solo se i
  conteggi di freschezza coincidono con quelli catturati;
- per l'indice, la classifica di ogni profilo, per dire chi sale e chi scende.
  Si accetta solo se il profilo Equilibrato coincide con la cattura.
...
def of_place(name: str, level: str) -> str:
    """La preposizione articolata davanti al territorio: "del Trentino Alto
    Adige", "della Sicilia", "di Reggio Calabria", "dell'Aquila"."""
    if level == "regione":
...
def region_key_of(row: dict, level: str, region_paths: dict) -> str | None:
    ...
    if level == "regione":
        return row["key"]
    path = (region_paths or {}).get(row.get("region") or "")
    return path.rsplit("/", 1)[-1] if path else None


def is_south(row: dict, level: str, region_paths: dict) -> bool | None:
    key = region_key_of(row, level, region_paths)
...
def table_rows(ranking: list[dict], level: str, region_paths: dict, with_delta: bool) -> list[dict]:
    ...
    for row in ranking:
        ...
        if level == "provincia" and row.get("region"):
            path = (region_paths or {}).get(row["region"])
            item["region"] = {"name": row["region"], "href": path}
        if with_delta:
...
def from_to(first: int, last: int) -> str:
    ...
    def art(prep: str, n: int) -> str:
        ...
        return f"{prep}ll'{helpers.ordinal(n)}" if vowel else f"{prep}lla {helpers.ordinal(n)}"
...
def positions(steps: int) -> str:
    return f"{helpers.count_word(steps, feminine=True)} {'posizione' if steps == 1 else 'posizioni'}"
...
def movers(ranking: list[dict], level: str, denominator: str | None = None) -> dict:
    ...
    for kind, pick in (("up", max), ("down", min)):
        ...
        if len(tied) == 1:
            ...
            text = (f"{many if is_plural else one} di più {who}, da {helpers.ordinal(r['rank'] + best)} a "
                    f"{helpers.ordinal(r['rank'])} {denominator}")
        elif len(tied) <= 3:
            who = join_names([subject(r["name"], level)[0] for r in tied])
            text = f"{many} di più, di {positions(steps)}, {who}"
        else:
            who = helpers.count_word(len(tied)) + ("" if short else f" {plural}")
...
def scored_years(level: str, data: dict) -> dict | None:
    ...
    `build_bes_ranking` non li restituisce: si leggono dalla stessa funzione
    dell'app da cui il punteggio parte. Si accettano solo se il numero degli
    indicatori e i conteggi di freschezza coincidono con la cattura, cosi' la
    pagina non mescola due versioni dei dati.
    """
...
def years_text(years: dict | None) -> str:
...
def years_span(years: dict | None) -> str:
...
def profile_rankings(level: str, profiles: list[dict], captured: list[dict]) -> dict[str, list[dict]] | None:
    """La classifica di ogni profilo, dall'app. None se il profilo Equilibrato
    non coincide con la classifica catturata."""
    from app.quality_life_bes import DEFAULT_PROFILE, build_bes_ranking

    with flask_app.app_context():
        out = {p["slug"]: (build_bes_ranking(level, p["slug"]) or {}).get("ranking") or [] for p in profiles}
    base = [(r["key"], r["score"]) for r in out.get(DEFAULT_PROFILE) or []]
    if base != [(r["key"], r["score"]) for r in captured]:
        return None
    return out
...
def sibling(name: str) -> dict:
    """Il contesto catturato di un'altra pagina del prototipo, o {} se manca."""
    try:
        return json.loads((DATA / f"{name}.context.json").read_text(encoding="utf-8")).get("context") or {}
...
def the_percent(value: float) -> str:
    ...
    return article + helpers.with_unit(value, "%")


def weights_text(profile: dict, categories: list[dict]) -> str:
    ...
    parts = [f"{lower_first(r['name'])} {helpers.with_unit(r['share'], '%')}" for r in ordered]
...
def breakdown(data: dict) -> list[dict]:
    ...
    for item in method.get("source_breakdown") or []:
        ...
        out.append({"label": label, "count": item.get("count")})
    return out
...
def category_table(data: dict, level: str, top: int = 3) -> list[dict]:
    ...
    for cat in measured_categories(data):
        ...
        out.append({
            "name": cat["name"], "href": category_path(cat["slug"]),
...
def champions_claim(data: dict, level: str) -> str | None:
    ...
    dims = f"{helpers.count_word(best, feminine=True)} dimensioni su {total}"
    if len(leaders) == 1:
        who, is_plural = subject(leaders[0], level)
        others = sorted({c for n, c in counts.items() if n != leaders[0]}, reverse=True)
        tail = ""
        if others:
            tail = f", nessun'altra {spec['singular']} in più di {helpers.count_word(others[0], feminine=True)}"
    ...
    dims = f"{helpers.count_word(best, feminine=True)} dimensioni ciascuna su {total}"
    return f"{who[:1].upper() + who[1:]} hanno il punteggio più alto in {dims}"


def south_split(ranking: list[dict], level: str, region_paths: dict) -> dict:
    """Centro-Nord e Mezzogiorno: quante righe, che media semplice del punteggio."""
    groups = {"north": [], "south": [], "unknown": []}
    for row in ranking:
        flag = is_south(row, level, region_paths)
...
def south_claim(ranking: list[dict], level: str, region_paths: dict) -> str | None:
    """Il titolo-affermazione della classifica, verificato sulle righe."""
    spec = LEVELS[level]
    groups = south_split(ranking, level, region_paths)
    ...
    if level == "regione":
        if {r["key"] for r in ranking[-k:]} == south_keys:
            return (f"Le {helpers.count_word(k)} regioni del Mezzogiorno occupano le ultime "
                    f"{helpers.count_word(k)} posizioni")
        below = [r for r in groups["south"]["rows"] if r["score"] < 50]
        return f"{helpers.count_word(len(below)).capitalize()} regioni del Mezzogiorno su {helpers.count_word(k)} stanno sotto 50"
    tail = sum(r["key"] in south_keys for r in ranking[-EDGE:])
    head = sum(r["key"] in south_keys for r in ranking[:EDGE])
    if tail == EDGE:
        text = f"Le ultime {helpers.count_word(EDGE)} {spec['plural']} sono tutte del Mezzogiorno"
    else:
        text = f"{helpers.count_word(tail).capitalize()} delle ultime {helpers.count_word(EDGE)} {spec['plural']} sono del Mezzogiorno"
    if head == 0:
        return text + f", nessuna fra le prime {helpers.count_word(EDGE)}"
    return text + f", {helpers.count_word(head)} fra le prime {helpers.count_word(EDGE)}"


def map_block(ranking: list[dict]) -> dict:
...
def derive(ctx: dict) -> dict:
    ...
    region_paths = ctx.get("region_paths") or {}
    method = data.get("methodology") or {}
    ...
    profiles = [{"name": p["name"], "href": profile_href(level, p["slug"], default), "current": p["slug"] == active}
    ...
    if missing:
        ...
        dims_note = (f"Per le {spec['plural']} mancano i dati {names}: "
                     f"i pesi si ridistribuiscono sulle altre {helpers.count_word(len(categories), feminine=True)} dimensioni.")
        dims_short = (f"Per le {spec['plural']} le dimensioni con dati sono "
                      f"{helpers.count_word(len(categories), feminine=True)}.")
    ...
    if level == "provincia":
        ...
        if unmeasured:
            coverage += (f" Nella codifica Istat ci sono anche {join_names(unmeasured)}, "
                         "che questi indicatori non coprono ancora.")
        coverage += " Le vecchie province sarde soppresse sono escluse."

    rows = table_rows(ranking, level, region_paths, with_delta=not is_default)
    ...
    return {
        ...
        "map": map_block(ranking) if level == "regione" and ranking and ranking[0].get("color") else None,
        ...
        "freshness_ok": years is not None,
        ...
        "citation": (f"Divario Italia, «{title}», elaborazione su dati {institutions} ({years_note}). "
                     f"{ctx.get('canonical')}"),
    }
