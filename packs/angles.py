...
Un giornalista guarda una serie e trova la storia. Quel passaggio è
calcolabile, ed è l'unico modo perché la struttura di un articolo nasca dai
dati invece che da uno scheletro fisso.

Ogni rilevatore restituisce zero o più **angoli**. Un angolo è un fatto
strutturale della serie, con la sua forza, le cifre che lo sostengono, i
territori e gli anni coinvolti, e la cautela che va detta se c'è. Non è prosa
da copiare: è materiale su cui costruire.

    {
      "type": "rottura-di-pendenza",
      "strength": 0.72,           # 0..1, confrontabile fra tipi diversi
      "figures": {...},           # le cifre citabili, già calcolate
      "diagnostica": {...},       # ciò che ordina l'angolo e non si scrive mai
      "territories": [...],
      "years": [2013],
      "caution": "..." | None,
    }

**La forza è confrontabile fra tipi diversi.** È la proprietà che rende
l'inventario ordinabile, ed è anche la più facile da sbagliare: ogni
...
scelta a occhio senza dire perché.

Stdlib pura per scelta: nessun import da `app`, nessun CSV, nessuna rete.
L'ingresso è una matrice `{anno: {territorio: valore}}` e basta, quindi i test
...
# quattro punti per tratto è il minimo sotto cui la retta interpola il rumore.
MIN_YEARS_FOR_TREND = 6
MIN_YEARS_PER_SEGMENT = 4
# Sotto dodici territori una dispersione descrive i pochi che rispondono, non
# il paese. Regge da sola, e va detto: il commento diceva "è la stessa soglia
# che `indicator_brief` usa per la correlazione", e `scripts/indicator_brief.py`
# è stato cancellato. Non esiste più un modulo gemello con cui allinearsi, e
# appellarsi a una coerenza non verificabile è il difetto, non la soglia.
MIN_TERRITORIES = 12

# La stessa domanda, per i rilevatori temporali, e non è lo stesso numero.
#
# `MIN_TERRITORIES` è assoluta perché i cinque rilevatori trasversali guardano
...
# minimo assoluto di dodici lì non misura niente, zittisce zero serie su trenta
# ed è una guardia solo in apparenza. Una **quota** dice la cosa che si voleva
...
# Una forza sotto questa non merita di essere proposta a chi scrive: è un
...
def _opposite_signs(one, other):
    """Se due quantità vanno da parti opposte. **Lo zero non va da nessuna parte.**

    `(a < 0) != (b < 0)` sembra la stessa cosa e non lo è: tratta lo zero come
    positivo, quindi risponde in due modi diversi a un fenomeno e al suo
    specchio. Il difetto è uscito due volte dalla stessa riga scritta due volte.
    ...
    tutti a zero, cioè l'angolo più forte del pacchetto costruito su chi non
    si è mosso. In `acceleration` una prima metà piatta dava `accelerazione`
    a 0,50 se la seconda saliva e silenzio se scendeva.

    Un'asimmetria così non si vede rileggendo: si vede solo provando il caso e
    il suo specchio, ed è per questo che i test qui vanno a coppie.
    """
    if one == 0 or other == 0:
        return False
...
def _scale(values):
    ...
    Lo scarto interquartile quando c'è, altrimenti l'escursione. Serve a
    confrontare un indicatore in euro con uno in punti percentuali senza che il
    primo vinca sempre per via dell'unità di misura.
    """
...
def _saturate(ratio):
    ...
    diverse. Serviva perché le prime formule tagliavano a 1,00 e producevano
    ...
    che è il prodotto di questo modulo, non ordinava più niente.
    """
    ratio = max(0.0, ratio)
...
def common_cohort(matrix):
    ...
    Vuoto se non ce n'è nessuno, ed è la risposta onesta: lì non esiste una
...
def covered_territories(matrix):
    """I territori che l'indicatore copre, cioè presenti in **almeno** un anno.

    È il denominatore della rappresentatività: la coorte va confrontata con
    ...
    anno parla per intero di ciò di cui dice di parlare, e una soglia sul
...
def cohort_is_representative(matrix):
    ...
    La domanda che `annual_means` di proposito non decide, decisa qui: è un
    rilevatore a doversi rifiutare di parlare, non una media a mentire. La media
    su quattro territori è esatta, la frase che la chiama "l'Italia" no.
    """
...
def annual_means(matrix):
    ...
    è l'aggregato nazionale ponderato, e chi scrive deve saperlo.

    **A coorte costante, e non era così.** La media si calcolava su chi
    ...
    coorte fissa. Una media che cambia perché cambia la popolazione non è un
    ...
    costante è `accelerazione` a 0,410, cioè il verso opposto. Il caso limite
    è `283`, venti territori in tutto e quattro presenti in ogni anno.

    È la stessa decisione che il canary del 2026-08-06 ha già preso (vedi
    ...
    Ciò che questa funzione **non** decide, e continua a non decidere: se
    quattro territori su venti bastino a parlare del paese. È una soglia di
    rappresentatività, una domanda diversa da quella della coorte, e la media
    resta esatta anche quando la coorte è sottile. A rifiutarsi di parlare sono
    i tre rilevatori che la leggono (`cohort_is_representative`), che è il posto
    giusto: così si perde un angolo, non si falsa una cifra. La coorte si
...
def _ranks(row, higher_first=True):
    """{territorio: posizione} per un anno, 1 è il primo. Pareggi: ordine stabile."""
...
# nel pacchetto, perché la calibrazione e i test le leggono, ma non arrivano
# mai a chi scrive: `packs/build.render` non le stampa.
#
# Non è una preferenza di stile, è una misura. Nella prima run i quattro
# giudici ciechi, indipendenti l'uno dall'altro, hanno indicato **tutti e
# quattro** come paragrafo più freddo quello che trascriveva pendenze di
# regressione e varianza spiegata. E l'angolo `rottura-di-pendenza`, che è
# quello che le produce, ha perso due giudizi su due.
#
# La regola per classificare: **diagnostica se il numero non è nell'unità
...
# precise. Invece "otto volte più largo del salto tipico" resta, perché è
...
def split_figures(figures):
    """(citabili, diagnostiche). L'unica porta fra le due, così si sposta in un posto solo."""
    citable, diagnostic = {}, {}
...
def _angle(kind, strength, figures, territories=(), years=(), caution=None):
    """Un angolo. `raw` è la dimensione dell'effetto, `strength` il suo posto.

    I due campi coincidono finché nessuno calibra: `find` sostituisce
    `strength` con la posizione dell'effetto nella distribuzione del proprio
    tipo, perché ordinare per dimensione grezza vuol dire ordinare per tipo.
...
def slope_break(matrix):
    ...
    una, e si tiene quello che riduce di più i residui. Forza: la quota di
    residuo che la rottura spiega, `1 - RSS(due rette) / RSS(una retta)`, che è
    zero quando la rottura non serve e tende a uno quando la serie è due rette
    diverse incollate.

    È il rilevatore che copre il caso più importante e oggi assente: una serie
    che cambia regime, che è quasi sempre la storia vera di un indicatore
    territoriale.
    """
    ...
    if gain < FLOOR or before is None or after is None:
        return []
    # Una pendenza nulla non ha un verso, quindi non può averlo cambiato. Vedi
    # `_opposite_signs`: qui `cambia_verso` è una cifra citabile, cioè una
    ...
    return [_angle(
        ...
        {"anno": year,
         ...
         "cambia_verso": turned,
         # La coorte su cui l'andamento è calcolato, dichiarata invece che
         # sottintesa. Vedi `annual_means`.
         "quanti_territori": len(common_cohort(matrix)),
         "quota_spiegata": round(gain, 3)},
        years=[year],
        caution="Una rottura nella serie può essere un cambio di metodo della "
...
def acceleration(matrix):
    """La serie va nella stessa direzione ma a un'altra velocità.

    Confronta la pendenza della prima metà con quella della seconda, a parità
    di verso. Forza: quanto la seconda pendenza si discosta dalla prima, in
    rapporto alla più grande delle due, così un raddoppio vale 0,5 e un
    fermarsi vale 1.

    Distinto da `slope_break`: lì la serie cambia regime in un anno preciso,
    ...
    if early is None or late is None:
        return []
    if early == 0:
        # Senza un verso di partenza non c'è nessuna "parità di verso" da
        # rispettare, e la formula misurerebbe una partenza invece di un cambio
        # di velocità. Il gradino piatto-poi-in-moto è materia di
        # `slope_break`, in tutte e due le direzioni: prima lo era in una sola.
        return []
    if _opposite_signs(early, late):
        return []  # cambio di verso: è materia di slope_break, non di velocità
...
def return_to_level(matrix, tolerance=0.02):
    """Il valore di oggi è quello di allora, dopo esserne uscito.

    Cerca l'anno più lontano il cui valore medio coincide con l'ultimo entro
    ...
    serie piatta produrrebbe l'angolo tutti gli anni, che è il modo in cui un
    rilevatore diventa rumore.

    Forza: quanto è ampia l'escursione in mezzo, rapportata alla scala, più un
...
def dispersion_trend(matrix):
    ...
    misura la pendenza. Forza: quanto è cambiato il divario fra il primo e
    l'ultimo anno, in rapporto al divario iniziale.

    Riporta anche **da che parte** si è chiusa, che è la metà della storia:
    un divario che si chiude perché il fondo sale non è un divario che si
    chiude perché la testa scende.
    """
    years = sorted(matrix)
    gaps = {}
    tops, bottoms = {}, {}
    ...
    if gaps[first] == 0:
        # Tutti i territori sullo stesso valore nel primo anno: non c'è un
        # divario di partenza, quindi non c'è una variazione relativa da
        # misurare, e qualunque formula sostitutiva sarebbe inventata.
        #
        # Non è la stessa cosa della guardia di `distribution_breaks`, che
        # invece taceva su un caso vivo (13 indicatori su 594). Questo è zero
        ...
        # anno una graduatoria spezzata, ed è quel rilevatore a raccontarla.
        return []
...
def rank_reversals(matrix, higher_first=True, top=3):
    ...
    anno è rumore, e raccontarlo come un cambiamento è un errore che le
    guardie numeriche non prendono, perché ogni singola cifra è giusta.
    ...
    for distance, held, one, other in found[:top]:
        # `distance` somma i due spostamenti, quindi può superare la lunghezza
...
def outliers(matrix, threshold=3.5):
    ...
    misura che dovrebbe scovarla. La soglia 3,5 sullo z modificato è quella di
    Iglewicz e Hoaglin.
...
def distribution_breaks(matrix, top=2):
    ...
    Il salto più largo fra due posizioni consecutive nell'ultimo anno. Forza:
    quanto quel salto è più grande del salto tipico, saturando a cinque volte.
    Così una graduatoria a scaletta regolare non produce nessun angolo, che è
    il comportamento giusto: lì non c'è nessuna spaccatura da raccontare.
    """
    years = sorted(matrix)
    if not years:
        return []
    ...
    if typical <= 0:
        # Metà o più delle posizioni sono in pari e la mediana dei salti è
        # zero. Rinunciare qui non è prudenza: è perdere la spaccatura più
        # netta che esista invece della più debole. Dieci territori a 100 e
        # dieci a 0 non hanno un salto tipico, hanno **un salto solo**, ed è
        # tutta la storia della serie.
        #
        # La media dei salti è positiva appena uno di loro lo è, e non misura
        # il salto contro sé stesso: la mediana dei soli salti positivi, che
    ...
    if typical <= 0:
        return []  # tutti allo stesso valore: non c'è nessuna graduatoria da spezzare
...
def against_the_grain(matrix):
    """Chi si è mosso contro la direzione generale.

    È il contro-esempio che impedisce di scrivere "è cresciuto ovunque"
    quando non è vero. Forza: la quota di territori controcorrente, saturata a
    un terzo, perché oltre quella soglia non c'è più una direzione generale
    da contraddire.
    """
    years = sorted(matrix)
    if len(years) < 2:
    ...
    if overall == 0:
        return []
    # Chi non si è mosso non si è mosso **contro**: vedi `_opposite_signs`. Il
    # denominatore resta `len(moves)` e non il numero di territori che si sono
    # mossi, perché la direzione generale la fanno tutti, fermi compresi.
...
def group_divergence(matrix, groups):
    ...
    `groups` è `{territorio: gruppo}`, tipicamente la macroarea. Si calcola la
    distanza fra le medie dei due gruppi più distanti nel primo e nell'ultimo
    anno. Forza: quanto è cambiata quella distanza, in rapporto a quella
    iniziale.

    Serve perché "il divario Nord-Sud" è l'affermazione che questo progetto fa
    più spesso, ed è quella che finora nessun rilevatore misurava: si guardava
    il primo contro l'ultimo territorio, che è un'altra cosa.
    """
    years = sorted(matrix)
    ...
    # **Il divario più largo di ciascun anno, misurato dentro quell'anno.**
    #
    # Prima i due gruppi si sceglievano una volta sola, sull'ordine dell'ultimo
    # anno, e lì con due soli gruppi non c'è differenza. Con tre o più sì,
    # ed è il caso normale: le macroaree italiane sono tre o cinque, non due.
    ...
    # impreciso: era il verso sbagliato, cioè il contrario della frase che
    # questo progetto fa più spesso.
    #
    ...
    # correzione precedente non serve più: non c'è più nessuna sottrazione
    ...
    # **Un ordine che non esiste non si può rovesciare.** Con tutti i gruppi
    ...
    # distanze a zero, cioè due gruppi che si scambiano un posto che nessuno
    ...
    # `None`, non zero, quando la distanza di partenza è zero: lì la variazione
    # relativa non è nulla, è **indefinita**, e scriverci 0,0 direbbe a chi
    # scrive che il divario non si è mosso mentre passava da zero a qualcosa.
    ...
    cifre = {
        ...
        # Chi stava agli estremi allora, per nome. Non è un di più: con tre
        # gruppi o più la coppia che fa il divario può cambiare, e un
        # articolo che dicesse "il divario si è allargato" senza dire fra chi
        ...
        # Il caso stretto: gli stessi due gruppi, ai posti scambiati. Lì "il
        # divario si è allargato" è vero e insieme fuorviante, perché chi sta
    ...
    if not forte_abbastanza:
        ...
        # tagliava l'angolo e con lui `ordine_invertito`, cioè la cifra spariva
        ...
        #
        # La cautela non dice "la distanza non si è mossa", e la differenza
        # conta: il ramo copre anche una distanza che cresce del 10%, cioè meno
        # del pavimento ma non zero, e una frase che la desse per ferma
        # contraddirebbe `variazione_relativa` due righe sopra.
        #
        # La forza è **fissa e dichiarata**, come per `method_breaks`, e non
        ...
        # (`QUANTILES`). Un quantile su un campione non è una misura, è un
        # numero che sembra una misura. 0,8 e non 0,9 perché quello è di
        # `rottura-di-metodo`, che non è una storia ma un vincolo su tutte le
        # altre; un sorpasso fra macroaree è la storia più forte che una serie
        ...
        return [_angle(
            ...
            caution="I due gruppi si sono scambiati di posto: chi stava sopra "
                    ...
                    f"{now:.3g}, e su quel movimento non c'è una storia da "
                    "raccontare: il fatto è il sorpasso. **Non scrivere che il "
                    "divario cresce o si chiude.** " + medie_semplici,
        )]

    # Quando la distanza si è mossa abbastanza, la storia resta il divario, e
    # l'inversione viaggia dentro `ordine_invertito`: sono i 13 casi su 14 in cui
    # il catalogo già portava il fatto a chi scrive.
    return [_angle(
...
def method_breaks(notes, years):
    ...
    `notes` è il testo libero della nota metodologica. Oggi lo legge a mano il
    verificatore, ed è costato almeno un articolo: `ter-60` aveva costruito la
    propria tesi sulla finestra prima di un cambio di metodo senza dirlo.

    Non è un rilevatore statistico, e la sua forza è fissa e alta: una rottura
    di metodo che cade dentro la finestra dell'articolo non è una storia fra le
    altre, è un vincolo su tutto il resto.
    """
    ...
    return [_angle(
        ...
        years=found,
        caution="La serie prima e dopo questa data può non essere la stessa "
...
#   ognuno di quei sorpassi è un angolo. Sono tutti "la Basilicata è salita".
#
# Tipi di famiglie diverse sopravvivono insieme anche sullo stesso territorio,
# perché lì i fatti sono davvero due: `sorpasso` e `controcorrente` sulla
...
def _suppress_repeats(angles):
    """Toglie l'angolo più debole che ripete una storia già raccontata.

    L'angolo tolto non sparisce: resta nominato in `implied_by` di quello che
    lo assorbe, così chi scrive sa che quella cifra è disponibile e sa anche
    che non è una seconda storia. La differenza fra le due cose è tutta la
...
def raw_angles(matrix, groups=None, notes=None, higher_first=True):
    ...
    Separato da `find` perché è ciò che serve alla calibrazione: per
...
def find(matrix, groups=None, notes=None, higher_first=True):
    """Tutti gli angoli di una serie, dal più forte al più debole.

    L'ordine è il prodotto: è quello che decide su che cosa aprirà
    l'articolo, e quindi che due indicatori diversi non aprano allo stesso modo.
    A parità di forza si ordina per tipo, così il risultato è deterministico
...
# Una rottura di metodo non è una storia: è un vincolo su tutte le altre.
# Modellarla come angolo era sbagliato, e si è visto appena l'ho collegata
...
def split_constraints(angles):
    ...
    l'articolo che attraversa una rottura senza dirlo è un errore che nessuna
    guardia numerica prende, perché ogni cifra è giusta da entrambe le parti.
    """
...
def total_strength(angles):
    """Quanta storia c'è in questo indicatore, in una cifra sola.

    Serve a decidere la lunghezza dell'articolo: la somma con rendimenti
    decrescenti (il secondo angolo vale metà del primo, il terzo un terzo)
    perché dieci angoli deboli non fanno un articolo lungo quanto tre forti.
    """
