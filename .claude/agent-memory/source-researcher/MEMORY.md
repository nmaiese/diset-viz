...
```yaml
- categoria: trappola di comparabilità temporale, monografie regionali
  apprendimento: >-
    ...
    dall'edizione sbagliata spiega un anno che non è quello del dato, ed è la
    prima delle smentite per un motivo.
...
- categoria: verifica di una citazione
  apprendimento: >-
    ...
    tutto. Due aggravanti misurate dopo: lo strumento tende a confermare una
    citazione che gli è già stata fornita nel prompt, anche quando è alterata,
    quindi il secondo fetch va fatto cieco, chiedendo il paragrafo intero
    attorno a una parola chiave senza passare la stringa cercata. E può
    restituire un riassunto tradotto di una pagina italiana, quindi l'assenza
    della frase nel testo che torna vale come non verificabile, mai come
    smentita: una citazione smentita ferma un articolo, una non verificabile no.
  ...
  limiti: >-
    chi non ha uno strumento per il grezzo deve passare la verifica al lead. Il
    fetch cieco ha trovato una citazione alterata su ter-167 (una proposizione
    staccata dal suo reggente), ma non prova l'assenza di alterazioni che non
    cambiano l'attacco della frase.
